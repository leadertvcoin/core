...
- [Development guidelines](#development-guidelines)
    - [General LeadERTVCoin Core](#general-leadertvcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
LeadERTVCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play LTVs (tLTV)" on the test network, if you
...
LeadERTVCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/leadertvcoin-project/leadertvcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_leadertvcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_leadertvcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/leadertvcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_leadertvcoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates LTVs (if wallet is enabled).
...
However, in open source software such as LeadERTVCoin Core, where everyone uses
...
pay attention to for reviewers of LeadERTVCoin Core code.

General LeadERTVCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: LeadERTVCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against LeadERTVCoin Core so that they can be integrated
...
file descriptors it will cause problems with LeadERTVCoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:LeadERTVCoin-Project/LeadERTVCoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `leadertvcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `leadertvcoin-cli` and the GUI debug console use this table to determine how to
