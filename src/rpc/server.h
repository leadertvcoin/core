...
/**
 * LeadERTVCoin RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZltvSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzltvseed(const UniValue& params, bool fHelp);
extern UniValue getzltvseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzltv(const UniValue& params, bool fHelp);
extern UniValue dzltvstate(const UniValue& params, bool fHelp);
