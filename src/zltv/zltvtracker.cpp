...
CzLTVTracker::CzLTVTracker(std::string strWalletFile)
{
...
CzLTVTracker::~CzLTVTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLTVTracker::Init()
{
...
bool CzLTVTracker::Archive(CMintMeta& meta)
{
...
bool CzLTVTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLTVTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLTVTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLTVTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzLTVTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzLTVTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzLTVTracker::GetSerialHashes()
{
...
CAmount CzLTVTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLTVTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLTVTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLTVTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLTVTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLTVTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLTVTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLTVTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLTVTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLTVTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLTVTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLTVWallet* zLTVWallet)
{
    bool iszLTVWalletInitialized = (NULL != zLTVWallet);
    CMintMeta meta;
    ...
    if (! iszLTVWalletInitialized)
        zLTVWallet = new CzLTVWallet(strWalletFile);
    meta.isSeedCorrect = zLTVWallet->CheckSeed(dMint);
    if (! iszLTVWalletInitialized)
        delete zLTVWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLTVTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLTVTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLTVTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLTVTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLTVTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLTVTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLTVWallet* zLTVWallet = new CzLTVWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zLTVWallet);
        }
        delete zLTVWallet;
        LogPrint("zero", "%s: added %d dzltv from DB\n", __func__, listDeterministicDB.size());
    }
