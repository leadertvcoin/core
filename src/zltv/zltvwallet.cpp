...
CzLTVWallet::CzLTVWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zltv seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLTVSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLTVSeed_deprecated()) {
                    LogPrintf("%s: Updated zLTV seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zltv seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zltv wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLTVWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLTVCount(nCountLastUsed);
    else if (!walletdb.ReadZLTVCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLTVWallet::Lock()
{
    seedMaster = 0;
}

void CzLTVWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLTVWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLTV(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLTVWallet::LoadMintPoolFromDB()
{
...
void CzLTVWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLTVWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLTVWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zltvTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLTVWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLTV(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zltvTracker which also adds to database
    pwalletMain->zltvTracker->Add(dMint, true);
...
void CzLTVWallet::SeedToZLTV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLTVWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLTVWallet::GenerateDeterministicZLTV(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLTVWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLTVWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLTVWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
