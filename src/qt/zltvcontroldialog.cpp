...
#include "ui_zltvcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zltvTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zltvTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zLTV.";
            else if (!mint.isSeedCorrect)
                strReason = "The zLTV seed used to mint this zLTV is not the same as currently hold in the wallet";
            else
