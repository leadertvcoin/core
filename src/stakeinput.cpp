...
// The zLTV block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zLTV is a hash of the serial
...
{
    //Create an output returning the zLTV that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZLTVOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLTV output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zLTV", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZLTVOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLTV output", __func__);
...
{
    CzLTVTracker* zltvTracker = pwallet->zltvTracker.get();
    CMintMeta meta;
    if (!zltvTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zltvTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a LTV stake is the outpoint
