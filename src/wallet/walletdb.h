...
public:
    ...
    bool WriteZLTVSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLTVSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLTVSeed_deprecated(uint256& seed);
    bool EraseZLTVSeed();
    bool EraseZLTVSeed_deprecated();

    bool WriteZLTVCount(const uint32_t& nCount);
    bool ReadZLTVCount(uint32_t& nCount);
